...
export  = (dependencies : any ): any =>{
    ...
    const loginUser = async (
        ...
        const isUserExist = await login_useCase(dependencies).interactor(userCredentials.email , userCredentials.password) 

        if(!isUserExist) return next (ErrorResponse.forbidden("No user Exist with this email , please Signup"))

        let userVerify = await comparePasswords(userCredentials.password , isUserExist.password)
