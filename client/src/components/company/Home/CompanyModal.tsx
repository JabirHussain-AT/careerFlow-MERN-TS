...
const CompanyModal: React.FC<CompanyModalProps> = ({ isModalOpen, setIsModalOpen }) => {
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (values.logo && typeof values.logo === "string") {
          ...
          const cloudinaryResponse = await fetch('https://api.cloudinary.com/v1_1/dato7wx0r/upload', {
            method: "post",
            body: formData,
          });
          ...
          // Dispatch action to handle the form data
          dispatch(companyForm(formik.values));
